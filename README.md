...
<a href="https://github.com/wegnerce/smk_rnaseq/releases/"><img src="https://img.shields.io/github/tag/wegnerce/smk_rnaseq?include_prereleases=&sort=semver&color=blue" alt="GitHub tag"></a>  <a href="#license"><img src="https://img.shields.io/badge/License-GPL3-blue" alt="License"></a> <a href="https://python.org" title="Go to Python homepage"><img src="https://img.shields.io/badge/Python-%3E=3.12-blue?logo=python&logoColor=white" alt="Made with Python"></a> <a href="https://doi.org/10.5281/zenodo.20445771"><img src="https://zenodo.org/badge/DOI/10.5281/zenodo.20445771.svg" alt="DOI"></a> <a href="https://snakemake.github.io"><img src="https://img.shields.io/badge/snakemake-≥9.21.0-brightgreen.svg" alt="SnakeMake">
# smk_rnaseq - A Snakemake-based workflow for RNAseq data processing

This workflow (v0.1) was used for RNAseq data processing in the following studies:
> * Gorniak et al., (2026) **[Source-independent enrichment of light lanthanides: microbial mobilization, selective uptake, and intracellular storage](https://doi.org/10.64898/2026.03.03.709224)** _bioRxiv_ 2026.03.03.709224v1 
> * Gorniak et al., (2024) **[Changes in growth, lanthanide binding, and gene expression in _Pseudomonas alloputida_ KT2440 in response to light and heavy lanthanides](https://doi.org/10.1128/msphere.00685-24)** _mSphere_ 9:e00685-24
...
[`Snakemake`](https://snakemake.github.io/) should be installed as outlined in its [documentation](https://snakemake.readthedocs.io/en/stable/getting_started/installation.html) for instance using [`pixi`](https://pixi.prefix.dev/latest/) or [`conda`](https://docs.conda.io/en/latest/miniconda.html)/[`mamba`](https://github.com/conda-forge/miniforge#mambaforge). 

In the case of using `conda`/`mamba`, it is recommended to create a dedicated `conda` environment for Snakemake.

## :blue_book: Description of the workflow
Single-end (since v0.2) or paired-end sequencing data (`stored in data/`) are first subjected to quality-control and adapter-trimming using `bbduk`. Quality reports are written using `fastQC` before and after trimming. mRNA-derived read pairs are identified by querying QCed reads against SSU and LSU databases using `SortMeRNA`. Next, mRNA-derived sequenced are mapped onto a reference genome with `bbmap`. Readcounts per feature (=CDS) are determined through `featureCounts`. The below DAG graph outlines the different processes of the workflow. The workflow includes all needed resources, except for the reference
genome as `.fasta`file and the `.saf` file needed by `featureCounts` to generate the final readcount data table. The later can either be pre-built, or generated during processing throught a provided conversion script `scripts/gff3_to_saf.py` (v0.2). `MultiQC` generates a report including the different workflow stages. The workflow is visualized below.
...
cd smk_rnaseq
```
`SortMeRNA` relies on sequence databases, the used SSU and LSU databases and the corresponding indices (only needed for v0.1) can be downloaded from [`zenodo`](https://zenodo.org/). Newer versions of `SortMeRNA` (including version 4.x implemented in v0.2 of the workflow) build the indices as needed on-the-fly.
```
cd resources
...
cd ..
```
Sequence data (R{1,2}.fastq.gz) should be stored under `data/`. The repository contains two pairs of exemplary files (La.1_R1.fastq.gz + La.1_R2.fastq.gz & Nd.1_R1.fastq.gz + Nd.1_R2.fastq.gz).
...
:copyright: Carl-Eric Wegner, 2023-2026
